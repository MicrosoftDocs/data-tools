...
| Analysis Services | When adding Azure Analysis Services administrators through **Server Properties** > **Security**, the **Search by name or email address** feature might not return results. | Add users by using the **Manual Entry** option. |
...
| Connection | You can't import connections from a previous version when using the modern connection dialog. This issue will be resolved in a later release. | There's no workaround. |
...
| GitHub Copilot in SSMS | When you set `environment.visualExperience.hideCopilotBadge` to `true` in the `settings.json` file, the Copilot badge isn't hidden. | Select **Hide Copilot** from the Copilot badge. |
| GitHub Copilot in SSMS | Agent mode currently does not support adding custom Agents. | Support will be added in a future release. |
| GitHub Copilot in SSMS | Plan mode is not available, even when **Enable Planning** is selected in **Tools** > **Options** > **GitHub** > **Copilot** > **Copilot Chat**. | Plan mode will be available in a future release. |
...
| New SQL Managed Instance link wizard | Starting with SSMS v22.7.0, you can preview adding multiple databases when creating a [Managed Instance link in SSMS](/azure/azure-sql/managed-instance/managed-instance-link-configure-how-to-ssms). This feature is temporarily disabled unless you request access. |For more information, see the [access request form](https://aka.ms/milink-multidb-prpr).|
