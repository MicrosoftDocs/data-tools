...
| --- | --- | --- |
| Allow single sign-on to SQL using Microsoft Entra authentication | **True**<br />**False** | When true, removes the requirement to provide full credentials to connect to Azure SQL Database and SQL Server using Microsoft Entra ID if using the same account used to sign in to Windows. |
...
| Graph Endpoint | `https://graph.microsoft.com/` | Specifies the URL for Microsoft Graph requests. |
...
| SQL Database Service Principal Name | `https://database.windows.net/` | The Azure SQL Database Service Principal Name (SPN) to obtain a token when using Microsoft Entra ID authentication. Also the audience of the JSON Web Token (JWT) for server-side JSON Web Token (JWT) parsing/validation. |
| Use system default web browser | **True**<br />**False** | When true, SSMS launches the user's default web browser to perform Azure logins. When false, SSMS uses an embedded browser. This option defaults to True with new installs starting with SSMS 19.1 |
...
| Microsoft Entra Endpoint Resource ID | `https://management.core.windows.net/` | Specifies the audience for tokens that authenticate requests to Azure Resource Manager (ARM) or classic deployment model (RDFE) endpoints. |
| Resource Manager Endpoint | `https://management.azure.com/` | Specifies the URL for Resource Management requests. |
| Service Endpoint | `https://management.core.windows.net/` | Specifies the endpoint for classic deployment model requests. |
...
| Option | Description |
| --- | --- |
| **Azure China Cloud** | International public cloud service generally available in the China market. For more information, see [Azure operated by 21Vianet documentation](/azure/china/). |
