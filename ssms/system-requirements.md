...
To download the latest version of [!INCLUDE [ssms-22-md](includes/ssms-22-md.md)], see [Install SQL Server Management Studio](install/install.md). For instructions on installing and updating [!INCLUDE [ssms-22-md](includes/ssms-22-md.md)], see:
...
[!INCLUDE [ssms-22-md](includes/ssms-22-md.md)] supports the following 64-bit operating systems:

- Windows 11 minimum supported OS version or higher: Home, Pro, Pro Education, Pro for Workstations, Enterprise, and Education, including Arm64 editions
...
Each supported operating system has its own lifecycle independent of the SSMS support lifecycle, which is based on the Visual Studio support lifecycle. Each lifecycle schedule informs adding and removing support for operating system version. An operating system might reach end of support before the SSMS release, at which point SSMS no longer considers that operating system version supported. If an operating system is in the Extended Security Updates (ESU) phase, it's supported only with ESU patches installed.

The following operating systems aren't supported:

- 32-bit and Arm32 operating systems.

- [Windows in S mode](https://support.microsoft.com/windows/windows-10-and-windows-11-in-s-mode-faq-851057d6-1ee9-b9e5-c30b-93baebeebc85), [Windows Enterprise IoT](/windows/iot/product-family/windows-iot), [Windows IoT Core](/windows/iot-core/windows-iot), [Windows Enterprise LTSC editions](/windows/deployment/update/waas-overview), and [Windows 10 Team Edition](https://techcommunity.microsoft.com/blog/surfaceitpro/updated-surface-hub-support-ends-for-windows-10%E2%80%94start-your-transition/4426867).

- Server IoT and Minimal Server Interface options for Windows Server.

- Windows containers.

- Virtual machine environments that aren't persistent, or don't have a full Windows operating system.<sup>1</sup>
...
- Multiple simultaneous users using the software on the same machine, including shared virtual desktop infrastructure machines or a multi-session Azure Virtual Desktop host pool.

<sup>1</sup> Running SQL Server Management Studio (SSMS) in a virtual machine environment requires a full Windows operating system. SSMS doesn't support multiple simultaneous users using the software on the same machine, including shared virtual desktop infrastructure machines or a multi-session Azure Virtual Desktop host pool.
