...
### View the report parameter values for a node in Object Explorer

1. Copy the following sample code into a new text file and name the file by using an .rdl extension.

1. Copy the report file to a folder that you created on the database server for custom reports.

1. In [!INCLUDE [ssManStudio](../includes/ssmanstudio-md.md)], right-click a node in Object Explorer, point to **Reports**, select Custom Reports. In the **Open File** dialog box, locate the custom reports folder and select the report file, and then select **Open**.
   ...
   1. To change the number of files displayed on the recently used list, on the **Tools** menu, select **Options**, expand the **Environment** folder, and then select **General**.

   1. Adjust the number for **Display files in recently used list**.

## Custom report code sample

The report that is created by using the following code uses the parameters that are associated with an Object Explorer node.
